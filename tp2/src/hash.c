...
size_t funcion_hash(const char* str, size_t capacidad_maxima){
    ...
    size_t hash = longitud_string * (size_t)str[longitud_string-1] + 31*(size_t)str[0] + 7*(size_t)str[longitud_string/2] - 1;
...
void liberar_string(void* elemento){ // Estas dos fueron "Tomadas prestadas" del hash_minidemo
  if(elemento){
    free(elemento);
  }
}

void mostrar_hash(hash_t* hash,char* str){  //Esta es una funcion que use para testear un par de cosas que se me complicaron. Imprime la tabla de hash y un sting para identifecar en donde la llame
    ...
    for(i = 0; i < hash->capacidad_maxima;i++){
        ...
        if(par_actual){
            if(par_actual->clave){
                elemento_actual = *((size_t*)par_actual->elemento);
...
int tabla_insertar(par_t** tabla,const char* clave, void* elemento, size_t capacidad_maxima,hash_destruir_dato_t destructor){
    ...
    char* clave_nueva = clonar_string(clave);
    if(!clave_nueva){
        free(par_a_agregar);
        return ERROR;
    }

    par_a_agregar->clave = clave_nueva;
    ...
    while(par_actual != NULL){
        if(par_actual->clave){
            if(strcmp(par_actual->clave, par_a_agregar->clave) == 0){
                ...
                liberar_string(par_actual->clave);
